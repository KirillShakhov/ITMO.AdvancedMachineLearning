...
print('1.3:', mean_persons.loc['Gerhard Schroeder'][0])
...
ab = mean_persons.loc[['Gerhard Schroeder', 'Hugo Chavez']].prod(axis=0).sum()
a = sqrt(mean_persons.loc[['Gerhard Schroeder', 'Gerhard Schroeder']].prod(axis=0).sum())
b = sqrt(mean_persons.loc[['Hugo Chavez', 'Hugo Chavez']].prod(axis=0).sum())
print('1.5:', ab / (a * b))
